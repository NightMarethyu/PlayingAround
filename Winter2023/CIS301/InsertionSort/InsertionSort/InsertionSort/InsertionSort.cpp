...
int main(int argc, char** argv)
{
    ...
    string path = "C:\\codeBase\\PlayingAround\\Winter2023\\CIS301\\InsertionSort\\";
...
int insertionSort() {
    ...
    for (int i = 1; i <= people.size() - 1; i++) {
        auto key = people.at(i);
        int j = i - 1;
        while (j >= 0 && people.at(j)->greaterthen(key)) {
            count++;
